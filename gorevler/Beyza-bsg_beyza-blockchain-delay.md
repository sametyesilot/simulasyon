# Konsensüs Gecikmesi / Blok Onay Anomalisi - Detaylı Saldırı ve Test Rehberi
================================================================================
...
Bu doküman, teknik bilgisi olmayan birinin bile **sıfırdan başlayarak** "Konsensüs Gecikmesi / Blok Onay Anomalisi" saldırısını kendi bilgisayarından nasıl gerçekleştireceğini adım adım anlatır.
...
1. `BSG_Test` klasörünün içinde `test_Beyza-bsg.py` adında yeni bir metin dosyası oluşturun (dosya uzantısının **.py** olduğuna emin olun, .txt kalmasın).
2. Dosyayı Notepad veya benzeri bir editörle açın.
3. Aşağıdaki kodları **KOPYALA - YAPIŞTIR** yapın:

```python
# Dosya Adi: test_Beyza-bsg.py
...
SENARYO_ID = "beyza-blockchain-delay"
...
**Beklenen Etki:**
- Loglarda: `High Block Time, Orphan Rate > 5%` benzeri uyarılar çıkmalı.
- Grafiklerde: Anormal veri artışları görülmeli.

---
*Bu doküman Beyza-bsg için özel olarak oluşturulmuştur.*
