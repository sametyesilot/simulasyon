# Off-Grid DC Bara Voltaj Manipülasyonu - Detaylı Saldırı ve Test Rehberi
================================================================================
...
Bu doküman, teknik bilgisi olmayan birinin bile **sıfırdan başlayarak** "Off-Grid DC Bara Voltaj Manipülasyonu" saldırısını kendi bilgisayarından nasıl gerçekleştireceğini adım adım anlatır.
...
1. `BSG_Test` klasörünün içinde `test_Yunus_BSG.py` adında yeni bir metin dosyası oluşturun (dosya uzantısının **.py** olduğuna emin olun, .txt kalmasın).
2. Dosyayı Notepad veya benzeri bir editörle açın.
3. Aşağıdaki kodları **KOPYALA - YAPIŞTIR** yapın:

```python
# Dosya Adi: test_Yunus_BSG.py
...
SENARYO_ID = "yunus-offgrid-voltage"
...
**Beklenen Etki:**
- Loglarda: `DC Voltage Spikes, Dump Load Inactive, Grid Instability` benzeri uyarılar çıkmalı.
- Grafiklerde: Anormal veri artışları görülmeli.

---
*Bu doküman Yunus_BSG için özel olarak oluşturulmuştur.*
